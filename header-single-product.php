...
<html class="no-js"  <?php language_attributes(); ?>>
	...
	<head>
		...
		<?php
			...
			$query = wc_get_product ( get_the_ID() );

			$image = get_the_post_thumbnail_url( get_the_ID(), 'full' );

			$term = wp_get_post_terms( get_the_ID(), 'pwb-brand' );
		?>
		
		<meta property="og:url"	content="<?php global $wp; echo home_url( add_query_arg( $_GET, $wp->request ) );  ?>" />
		<meta property="og:type" content="website" />
		<meta property="og:title" content="<?php echo "Get " . $term[0]->name . " " . $query->get_title() . " at Shop TGC" ; ?>" />
		<meta property="og:description" content="<?php echo $query->get_short_description(); ?>" />
